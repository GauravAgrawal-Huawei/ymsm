...
package org.onosproject.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network4.rev20171208.ietfnetwork4;
...
public class DefaultNetwork implements Network {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(ip, onosYangNodeOperationType);
    }
    ...
    private boolean processLeafSubtreeFiltering(Network appInstance, NetworkBuilder subTreeFilteringResultBuilder,
        ...
        if (getValueLeafFlags().get(LeafIdentifier.IP.getLeafIndex())) {
            if (appInstance.ip() != ip()) {
                return false;
            } else {
                subTreeFilteringResultBuilder.ip(appInstance.ip());
            }
        } else if (getSelectLeafFlags().get(LeafIdentifier.IP.getLeafIndex()) || isSelectAllSchemaChild) {
    ...
    public static class NetworkBuilder implements Network.NetworkBuilder {
        ...
        @Override
        public NetworkBuilder ip(int ip) {
            getValueLeafFlags().set(LeafIdentifier.IP.getLeafIndex());
            this.ip = ip;
            return this;
        }
